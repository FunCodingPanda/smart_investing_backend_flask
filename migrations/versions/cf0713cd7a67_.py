...
def upgrade():
    transaction_type = postgresql.ENUM('buy', 'sell', 'dividend', name='transactiontype')
    transaction_type.create(op.get_bind())
    op.add_column('transactions', sa.Column('type', sa.Enum('buy', 'sell', 'dividend', name='transactiontype'), nullable=True))
